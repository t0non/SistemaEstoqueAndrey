  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-4">
        <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.95 }} className="md:col-span-1">
        <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
          <CardHeader className="space-y-0 py-1">
          ...
          <CardContent className="flex flex-col justify-center">
            {isLoading ? (
              ...
              <div className="text-2xl sm:text-3xl font-semibold tracking-[-0.02em] text-emerald-600">
                {formatCurrency(analyticsData.todayStats.revenue)}
              </div>
            )}
            {!isLoading && (
              <div className="mt-1 text-xs font-medium">
                <span className={analyticsData.periodDeltas.revenue >= 0 ? 'text-emerald-600' : 'text-red-600'}>
                  {`${analyticsData.periodDeltas.revenue >= 0 ? '+' : ''}${Math.round(analyticsData.periodDeltas.revenue)}%`}
                </span>
        ...
        <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
          <CardHeader className="space-y-0 py-1">
            <CardTitle className="text-sm font-medium">Custo (CMV hoje)</CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col justify-center">
             {isLoading ? (
              <Skeleton className="h-8 w-3/4" />
            ...
            <div className="text-2xl sm:text-3xl font-semibold tracking-[-0.02em]">
              {formatCurrency(analyticsData.todayStats.cost)}
            </div>
            )}
            {!isLoading && (
              <div className="mt-1 text-xs font-medium">
                <span className={analyticsData.periodDeltas.cost <= 0 ? 'text-emerald-600' : 'text-red-600'}>
                  {`${Math.round(analyticsData.periodDeltas.cost)}%`}
                </span>
                <span className="text-slate-500"> vs período anterior</span>
              </div>
            )}
        ...
        <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
          ...
          <CardContent className="flex flex-col justify-center">
            ...
            <div className="text-2xl sm:text-3xl font-semibold tracking-[-0.02em] text-red-700">
              {formatCurrency(analyticsData.todayStats.expenses)}
            </div>
            )}
            {!isLoading && (
              <div className="mt-1 text-xs font-medium">
                <span className={analyticsData.periodDeltas.expenses <= 0 ? 'text-emerald-600' : 'text-red-600'}>
                  {`${Math.round(analyticsData.periodDeltas.expenses)}%`}
                </span>
                <span className="text-slate-500"> vs período anterior</span>
              </div>
            )}
        ...
        <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.95 }} className="lg:col-span-2">
        <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
          <CardHeader className="space-y-0 py-1">
            <CardTitle className="text-sm font-medium">
              Lucro Líquido (hoje)
          ...
          <CardContent className="flex flex-col justify-center">
            ...
            <div className="text-3xl sm:text-4xl font-semibold tracking-[-0.02em] text-slate-800">
              {formatCurrency(analyticsData.todayStats.profit)}
            </div>
            )}
            {!isLoading && (
              <div className="mt-1 text-xs font-medium">
                <span className={analyticsData.periodDeltas.profit >= 0 ? 'text-emerald-600' : 'text-red-600'}>
                  {`${analyticsData.periodDeltas.profit >= 0 ? '+' : ''}${Math.round(analyticsData.periodDeltas.profit)}%`}
                </span>
                <span className="text-slate-500"> vs período anterior</span>
              </div>
            )}
        ...
        <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-300">
          ...
          <CardContent className="flex flex-col justify-center">
            {isLoading ? (
              ...
              <div className="text-2xl sm:text-3xl font-semibold tracking-[-0.02em]">
                {formatCurrency(analyticsData.todayAvgTicket)}
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {/* Chart */}
      <Card className="bg-white/70 backdrop-blur-2xl saturate-150 border border-white/40 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        <CardHeader>
