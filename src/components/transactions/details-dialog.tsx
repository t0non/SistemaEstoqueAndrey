    ...
    try {
      ...
      await runTransaction(firestore, async (tx) => {
        ...
        for (let i = 0; i < freshItems.length; i++) {
          if (!productSnapshots[i].exists()) {
            throw new Error(`Produto "${freshItems[i].productName}" não encontrado.`);
          }
        }
        
        for (let i = 0; i < freshItems.length; i++) {
          const productRef = doc(firestore, 'products', freshItems[i].productId);
          const productData = productSnapshots[i].data() as any;
          const currentStock = productData?.currentStock || 0;
          const stockAdjustment =
            transaction.type === 'OUT'
              ? freshItems[i].quantity
              : -freshItems[i].quantity;
      ...
      try {
        ...
        for (const item of freshItems) {
          ...
          const currentStock = productData?.currentStock || 0;
          const stockAdjustment =
            transaction.type === 'OUT' ? item.quantity : -item.quantity;
          const newStock = currentStock + stockAdjustment;
